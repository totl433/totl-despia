...
export default function FloatingTabBar({ state, navigation }: BottomTabBarProps) {
  ...
  const indicatorStyle = {
    width: itemWidth,
    height,
    borderTopLeftRadius: isFirst ? radius : 72,
    borderBottomLeftRadius: isFirst ? radius : 72,
    borderTopRightRadius: isLast ? radius : 72,
    borderBottomRightRadius: isLast ? radius : 72,
    ...
    >
      ...
      >
        ...
        {WEB_TABS.map((tab, idx) => {
          ...
          const iconEl = tab.renderIcon({ color });
          ...
          return (
            <Pressable
              ...
              onPress={() => {
                if (!route) return;
                // Native behavior: tapping the active tab scrolls to top (useScrollToTop listens to tabPress).
                ...
                // If this tab is already active, don't navigate again (the tabPress event is enough).
                if (isActive) {
                  // Special case: always return "Mini Leagues" to the list root if somehow not there.
