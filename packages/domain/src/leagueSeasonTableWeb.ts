...
type PickRow = {
  ...
  pick: 'H' | 'D' | 'A';
  /** Present only on paged diagnostic loads; not used for Season scoring. */
  created_at?: string | null;
};
...
export type MiniLeagueSeasonPreload = {
  members?: Array<{ id: string; name: string; created_at?: string | null }> | null;
  league?: { id: string; name?: string | null; created_at?: string | null; start_gw?: number | null } | null;
};

const DEADLINE_BUFFER_MINUTES = 75;

function parseOptionalGwColumn(value: unknown): number | null {
...
function isIsoDateWebStyle(value: unknown): value is string {
...
async function fetchLeagueRowForSeason(supa: any, leagueId: string) {
  ...
  return res.data as { id: string; name?: string; created_at?: string | null; start_gw?: unknown } | null;
}

async function ensureLeagueMeta(supa: any, league: LeagueRecord): Promise<LeagueRecord> {
  ...
  if (!needsName && !needsCreatedAt && !needsStartGw) return league;
  ...
  if (sel.error || !sel.data) return league;

  const data = sel.data as { name?: string; created_at?: string | null; start_gw?: unknown };
...
export async function orderCompletedGwsByFirstKickoff(supa: any, completedGws: number[]): Promise<number[]> {
  ...
  const earliestMsByGw = new Map<number, number>();
  for (const row of data as { gw?: number | null; kickoff_time?: string | null }[]) {
    const gw = Number(row.gw);
    if (!Number.isFinite(gw) || typeof row.kickoff_time !== 'string') continue;
    const t = new Date(row.kickoff_time).getTime();
    if (!Number.isFinite(t)) continue;
    const prev = earliestMsByGw.get(gw);
    if (prev === undefined || t < prev) earliestMsByGw.set(gw, t);
  }
...
export async function resolveLeagueStartGwWeb(
  supa: any,
  ...
  if (anchorTs && currentGw) {
    ...
    for (const gw of completedOrdered) {
      ...
      if (firstFixture?.kickoff_time) {
        ...
        if (anchorTime < deadlineTime) return gw;
      }
    }
...
function emptyRows(memberList: Member[]): LeagueSeasonTableRow[] {
...
export async function computeWebParityMiniLeagueSeasonRows(
  ...
  const leagueStartGw =
    typeof hintGw === 'number' && Number.isFinite(hintGw) && hintGw >= 1
      ? Math.trunc(hintGw)
      : await resolveLeagueStartGwWeb(
          supa,
          {
            id: league.id,
            name: league.name,
  ...
  const rows: LeagueSeasonTableRow[] = members.map((m) => ({
    user_id: m.id,
    name: m.name,
...
async function computeRelevantGwsWindow(
  supa: any,
  ...
  const chunks = await Promise.all(
    memberIds.map(async (uid) => {
      ...
      for (let offset = 0; ; offset += APP_PICKS_PAGE_SIZE) {
        ...
        out.push(...batch);
        if (batch.length < APP_PICKS_PAGE_SIZE) break;
      }
      return out;
    })
  );
