...
| Expo (Metro) | `npm run mobile:dev` (or `cd apps/mobile && npm run start`) | 8081 | Press `w` for web, `i` for iOS sim. Use `npm run mobile:dev:tunnel` if simulator can't connect. |
...
### Metro "Could not connect to development server" (iOS Simulator)

If the simulator shows this error (often with a stale IP like `192.168.x.x:8081`):

1. **Fully quit the app** in the simulator (swipe up to close, don't just background it).
2. **Restart Metro** with `npm run mobile:dev` (uses `--localhost` and `REACT_NATIVE_PACKAGER_HOSTNAME=127.0.0.1`).
3. **Launch fresh from Expo**: press `i` in the Metro terminal to open the app (don't tap the app icon manually).
4. If it still fails, use **tunnel mode**: `npm run mobile:dev:tunnel`, then press `i`. Slower but works when LAN/localhost fails.
