...
export default function GetAppPage() {
  ...
  return (
    <div className="fixed inset-0 z-[100] flex w-full items-stretch justify-center overflow-hidden bg-white">
      {/*
        fixed inset-0 + items-stretch: fill the iPhone viewport.
        items-center + 100svh was clipping slide titles and leaving a shell-coloured gap under the deck.
      */}
      <div className="get-app-column relative z-10 flex h-full min-h-0 w-full max-w-[430px] flex-col overflow-hidden bg-white sm:my-6 sm:h-[min(760px,calc(100dvh-3rem))] sm:max-h-[calc(100dvh-3rem)] sm:self-center sm:rounded-[1.75rem] sm:border sm:border-black/[0.06] sm:shadow-[0_20px_50px_rgba(0,0,0,0.08)]">
        <div
          ref={scrollerRef}
          ...
          {SLIDES.map((slide) => {
              ...
              return (
                ...
                >
                  ...
                  <div
                    ...
                    aria-hidden
                  />
                  <div className="relative z-10 flex h-full flex-col px-5 pb-[max(7rem,calc(env(safe-area-inset-bottom)+5.5rem))] pt-[max(2rem,env(safe-area-inset-top))]">
            ...
            if (slide.kind === 'cta') {
              return (
                ...
                >
                  <div className="flex h-full w-full flex-col items-center justify-center px-6 pb-[max(7rem,calc(env(safe-area-inset-bottom)+5.5rem))] pt-[max(1.75rem,env(safe-area-inset-top))] text-center">
                    <img
                      src="/assets/badges/totl-logo1.svg"
                      alt="TotL"
                      className="h-[84px] w-auto"
                    />
                    <h2 className="mt-5 text-[1.65rem] font-semibold leading-tight tracking-tight text-[#0a1224]">
                      {slide.title}
                    </h2>
            ...
            return (
              <section
                ...
                className="relative box-border flex h-full w-full flex-[0_0_100%] snap-center snap-always flex-col overflow-hidden bg-white"
              >
                <div className="flex h-full w-full flex-col px-5 pb-[calc(10.5rem+env(safe-area-inset-bottom,0px))] pt-[max(2.75rem,calc(env(safe-area-inset-top)+1.25rem))]">
                  <div className="relative z-10 shrink-0">
                    <h2 className="text-[1.65rem] font-semibold leading-tight tracking-tight text-[#0a1224]">
                      {slide.title}
                    </h2>
                    <p className="mt-2.5 text-[15px] leading-relaxed text-black/60">{slide.body}</p>
                  </div>
        ...
        {/* Dots — same absolute bottom on every slide */}
        <div
          className={`pointer-events-none absolute inset-x-0 bottom-0 z-30 px-5 pb-[max(1rem,env(safe-area-inset-bottom))] ${
            onSplash
              ? 'bg-gradient-to-t from-black/55 via-black/20 to-transparent pt-14'
              : onCta
                ? 'bg-gradient-to-t from-white via-white/90 to-transparent pt-10'
                : ''
          }`}
        >
          <div
