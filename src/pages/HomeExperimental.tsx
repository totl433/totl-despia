...
export default function HomeExperimental() {
  ...
  const loadInitialStateFromCache = () => {
    ...
    try {
      ...
      if (cached && cached.currentGw) {
        ...
        const leagueDataCacheKey = `home:leagueData:v2:${userId}:${cached.currentGw}`; // v2: Added seasonLeaderName
  ...
  useEffect(() => {
    ...
    let alive = true;
    const leagueDataCacheKey = `home:leagueData:v2:${user.id}:${gw}`; // v2: Added seasonLeaderName
