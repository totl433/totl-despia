...
async function checkDGPicksTimestamps() {
  ...
  if (picksData && picksData.length > 0) {
    ...
    if (!allSameCreated) {
      console.log(`   ⚠️  Picks were created at DIFFERENT times!`);
      picksData.forEach(p => {
    ...
    if (submissionData && submissionData.submitted_at) {
      ...
      if (latestUpdated && latestUpdated.getTime() > submissionTime) {
        console.log(`   🚨 PICKS WERE UPDATED AFTER SUBMISSION!`);
        console.log(`   Latest update: ${latestUpdated.toISOString()}`);
        console.log(`   Submission: ${new Date(submissionTime).toISOString()}`);
        console.log(`   Difference: ${Math.round((latestUpdated.getTime() - submissionTime) / 1000 / 60)} minutes`);
  ...
  // Also check app_picks for comparison
  console.log('\n📊 Checking app_picks table for comparison:');
  ...
  if (!appPicksError && appPicksData && appPicksData.length > 0) {
    const appCreatedTimes = appPicksData.map(p => new Date(p.created_at).getTime());
    const appUpdatedTimes = appPicksData.map(p => p.updated_at ? new Date(p.updated_at).getTime() : null);
    
    const appEarliestCreated = new Date(Math.min(...appCreatedTimes));
    const appLatestUpdated = appUpdatedTimes.filter(t => t !== null).length > 0 
      ? new Date(Math.max(...appUpdatedTimes.filter(t => t !== null)))
      : null;
    
    console.log(`   Earliest created_at: ${appEarliestCreated.toISOString()}`);
    if (appLatestUpdated) {
      console.log(`   Latest updated_at: ${appLatestUpdated.toISOString()}`);
    } else {
      console.log(`   No updated_at timestamps`);
    }
  }
}
